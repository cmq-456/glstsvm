...
LIB_DOC_PAGE = "https://libtwinsvm.readthedocs.io/en/latest/"
...
#                                            self.get_save_pred_path(False))
        ...
        self.rect_kernel_rbtn.toggled.connect(self.rect_kernel_percent.
                                              setEnabled)
        self.rect_kernel_rbtn.toggled.connect(lambda checked: not checked and
                                              self.rect_kernel_percent.
                                              setEnabled(False))
        ...
        # Help menu
        self.actionDocumentation.triggered.connect(lambda: webbrowser.
                                                   open(LIB_DOC_PAGE))
    ...
    def get_save_path(self):
        ...
        results_path = QFileDialog.getExistingDirectory(self, "Select results'"
                                                        " directory")
    ...
    def load_data(self):
        ...
        self.data_reader = DataReader(self.path_box.text(), self.sep_char_box.
                                      text(), True if self.header_check.
    ...
    def gather_usr_input(self):
        ...
        if self.cv_rbtn.isChecked():
            ...
            self.user_in.test_method_tuple = ('t_t_split', (100 -
                                              self.tt_percentage.value()) /
                                              100)

        self.user_in.C1_range = (self.C1_lbound.value(),
                                 self.C1_ubound.value())
        self.user_in.C2_range = (self.C2_lbound.value(),
                                 self.C2_ubound.value())

        if self.rbf_kernel_rbtn.isChecked() or \
           self.rect_kernel_rbtn.isChecked():

            self.user_in.u_range = (self.u_lbound.value(),
                                    self.u_ubound.value())
        ...
        self.user_in.save_clf_results = (True if self.clf_results_chk.
                                         isChecked() else False)
        self.user_in.save_best_model = (True if self.best_mode_chk.isChecked()
                                        else False)
        ...
        if self.validate_usr_input():
            ...
            ConfrimDialog(self.user_in.get_current_selection(),
                          self.run_gs_thread)
    ...
    def validate_usr_input(self):
        ...
        if self.user_in.validate_step_size():
            ...
            show_dialog("Invalid Step Size", "Step size exceeds the chosen "
                        "range for hyper-parameters.", QMessageBox.Warning)
        ...
        if any([self.user_in.save_clf_results, self.user_in.save_best_model,
            ...
            if validate_path(self.user_in.result_path):
                ...
                total_valid.append(False)
                show_dialog("Invalid Save Path", "The path for saving "
                            "classification results does not exist.",
                            QMessageBox.Warning)
    ...
    def log_file_info(self):
        ...
        if self.log_file_chk.isChecked():
            ...
            show_dialog("Logging Results", "Note that logging classification"
                        ...
                        "results will be not lost in case of power failure, "
                        "etc.", QMessageBox.Information)
    ...
    def stop_gs_thread(self, finished=False):
        ...
        if not finished:
            ...
            show_path = ''
            if any([self.user_in.save_clf_results,
                    self.user_in.save_best_model, self.user_in.log_file]):
                    show_path = ("If selected, the results, model, and log "
                                 "file are saved at:\n%s\n" %
                                 self.user_in.result_path)

            show_dialog("Done!", "The grid search is finished!\n%s" %
                        (show_path), QMessageBox.Information)
    ...
    def get_model_path(self):
        ...
        model_filename, _ = QFileDialog.getOpenFileName(self, "Import "
                                                        "pre-trained model",
                                                        "", "Model files "
                                                        "(*.joblib)")
    ...
    def get_save_pred_path(self):
        ...
        if self.model_pred_chk.isChecked():
            ...
            pred_file_path = QFileDialog.getExistingDirectory(self, "Select a "
                                                              "directory")
    ...
    def load_model_info(self):
        ...
        self.user_in.pre_trained_model, \
        model_info = load_model(self.model_path_box.text())
    ...
    def eval_model_test(self):
        """
        Checks required fields and then runs model evaluation in its own
        thread.
        """
