...
class Validator:
    ...
    W: array-like, shape (n_samples,300)   
    ...
    def __init__(self, X_train, X_test, y_train, y_test, W, W11, W22, validator_type, estimator):  
    ...
    def tt_validator(self, dict_param):
        """
        It evaluates a TSVM-based estimator using the train/test split method.它使用train/test split方法评估基于tsvm的估计器。
        ...
        self.estimator.fit(self.W11, self.W22, self.X_train, self.y_train)

        output = self.estimator.predict(self.X_test)
        #print(output)

        tp, tn, fp, fn, accuracy, recall_p, precision_p, f1_p, recall_n, \
        precision_n, f1_n = eval_metrics(self.y_test, output)
    ...
    def tt_validator_mc(self, dict_param):
        ...
        self.estimator.fit(self.X_train, self.y_train, self.W)
        
        output = self.estimator.predict(self.X_test)
        #print(output)
        acc = accuracy_score(self.y_test, output) * 100

        return acc, 0.0, {**{'accuracy': acc, 'acc_std': 0.0,
               'micro_recall': recall_score(self.y_test, output, average='micro') * 100,
               'm_rec_std': 0.0, 'micro_precision': precision_score(self.y_test,
               output, average='micro') * 100, 'm_prec_std': 0.0, 'mirco_f1':
               f1_score(self.y_test, output, average='micro') * 100, 'm_f1_std': 0.0},
               **dict_param}
    ...
    def choose_validator(self):
        ...
        if isinstance(self.estimator, BaseTSVM):
            ...
            if self.validator == 'CV':

                return self.cv_validator

            elif self.validator == 't_t_split':
...
def save_result(validator_obj, problem_type, gs_result, output_file):
    ...
    result_frame = pd.DataFrame(gs_result,
                   columns=column_names[problem_type][validator_obj.validator] + \
                   param_names)
...
class ThreadGS(QObject):
    ...
    def run_gs(self, func_eval, search_space):
        ...
        self.sig_pbar_set.emit(search_total)
